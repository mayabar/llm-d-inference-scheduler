...
func NewScheduler(ctx context.Context, schedCfg *config.Config, ds Datastore) (*Scheduler, error) {
	...
	scheduler.prefill = scheduling.NewSchedulerWithConfig(
		ds,
		scheduler.generateSchedulerConfig(ctx, schedCfg.PrefillSchedulerPlugins, schedCfg.PrefillSchedulerExternalPlugins,
			&filter.PrefillFilter{}),
	)

	scheduler.decode = scheduling.NewSchedulerWithConfig(
		ds,
		scheduler.generateSchedulerConfig(ctx, schedCfg.DecodeSchedulerPlugins, schedCfg.DecodeSchedulerExternalPlugins,
			&filter.DecodeFilter{}),
	)
